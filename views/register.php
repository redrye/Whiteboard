...
echo "<div class='card-body'><table class='table table-striped table-bordered datatable'><tr><th>Course ID</th><th>Class</th><th>Instructor</th><th></th></tr>";
...
if ($result = $conn->query($sql)) {
	if($result->num_rows > 0) {
    	...
    	while($row = $result->fetch_assoc()) {
    	    echo "<tr><td>".$row['crn']."</td><td>".$row['name']."</td><td>".$row['instructor']."</td><td><input type='hidden' name=crn value='".$row['crn']."'><ul class='nav'><li class='nav-item'><a class = 'nav-link btn-primary' href='register_class.php?crn=".$row['crn']."'>Add</a></li></ul></td></tr>";
    	}
    	echo "</tbody></table>";
